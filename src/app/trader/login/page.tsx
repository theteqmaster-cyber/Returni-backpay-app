  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.role !== 'trader' && data.role !== 'admin') {
         throw new Error('Not authorized as trader');
      }

      localStorage.setItem('returni_trader_id', data.trader_id || '');
    ...
    <main className="min-h-screen p-6 bg-returni-bg flex flex-col items-center justify-center">
      <div className="w-full max-w-md bg-white p-8 rounded-3xl shadow-xl border border-gray-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          >
            {loading ? 'Entering Portal...' : 'Access My Dashboard'}
          </button>
        </form>

        <div className="mt-8 pt-8 border-t border-gray-100 text-center">
           <p className="text-xs text-gray-400 font-bold uppercase tracking-widest leading-relaxed">
             Manage multiple locations with<br/>aggregated live analytics.
           </p>
        </div>
      </div>
    </main>
